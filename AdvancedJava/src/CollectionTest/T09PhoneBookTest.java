...
public class T09PhoneBookTest {
	...
	private void insert() { // 새로운 전화번호 등록 메소드
		System.out.println();
		System.out.println("새로 등록할 정보를 입력하세요");
		System.out.print("이름 : ");
		String name = scan.next();
		if (phoneBookMap.get(name) != null) { // 중복데이터 검사
	...
	private void delete() { // 회원정보 삭제
		System.out.println();
		System.out.println("정보를 삭제할 사람의 이름을 입력하세요");
		System.out.print("이름 : ");
		String name = scan.next();
		if (phoneBookMap.remove(name) == null) { // 중복데이터 검사
			System.out.println(name + "님과 관련된 정보가 없습니다.");
			return; // 메소드 종료
		} else {
	...
	private void search() { // 회원의 상세정보를 확인
		System.out.println();
	...
	private void displayAll() {// 전체 자료 출력

...
class RacingThread extends Thread {
	...
	public void run() {
		...
		X_HorseRacing_X.rankCnt++;
		if (X_HorseRacing_X.rankCnt >= HorseRacing.horse.size()) { // 총 10마리 경주 완료시
			X_HorseRacing_X.end = true;
		}

		for (int i = 0; i < X_HorseRacing_X.horse2.size(); i++) {
			if (X_HorseRacing_X.horse2.get(i).getName().equals(name)) { // 경주가 종료된 말과 이름이 같은 horse객체에
				X_HorseRacing_X.horse2.get(i).setRank(X_HorseRacing_X.rankCnt);// 등수 저장
			}
		}
	}
...
//Horse 클래스
class Horse2 implements Comparable<Horse2> {
